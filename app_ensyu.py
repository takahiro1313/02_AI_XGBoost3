...
「今月の活動 → 来月の成約数」を予測するAI。
**書く場所は手元の `数値AI_演習.py`**（全体が見える平文。VSCode等で書く）。
書けたら、その中身を配布URL（このアプリ）のコード欄に**貼り付けて実行**＝計算は **Cloud（サーバ側）** で動く。
ポイント：AIに見せる「説明変数」を、`問い合わせ数`・`面談数` などの**活動の名前で**書く。
...
# =========================================================================
ACT_COLS = ["問い合わせ数", "架電数", "面談数", "案件提案数"]  # 説明変数の候補（活動）
TARGET = "翌月の成約数"
...
# --- コールバック（学習・推論） ------------------------------------------
def _do_learn():
    """Step2 の編集セル（説明変数＋モデル＋fit）を実行して学習する。"""
    ...
    if err is None and (説明変数 is None or not hasattr(説明変数, "columns")):
        err = NameError("説明変数 が空です。活動の列名（例: \"案件提案数\"）を書いてね。")
    if err is None and (目的変数 is None or not hasattr(目的変数, "name")):
        err = NameError("目的変数 を書いてね（\"翌月の成約数\"）。")
    if err is not None:
        st.session_state.fit_err = str(err)
        st.session_state.model = None
        return
    model = ns["model"]
    cols, target = list(説明変数.columns), 目的変数.name
...
def _do_predict():
    ...
    ns = {"model": st.session_state.model, "来月の説明変数": last[cols]}
    ...
    予測 = ns.get("予測")
    if 予測 is None:
        st.session_state.pred_err = "予測 に結果が入っていません。書き方を見直してください。"
        st.session_state.prediction = None
        return
    st.session_state.pred_err = None
    st.session_state.prediction = float(予測[0])
...
| | ステップ | あなたの操作 |
|---|---|---|
| 🔒 | Step1 データを見る | 見るだけ（説明変数＝活動／答え＝翌月の成約） |
| 🟢 | Step2 学習コードを書く | 手元の `数値AI_演習.py` に書く → **貼り付けて** 🎓 学習 |
| 🟢 | Step3 推論コードを書く | 担当を選ぶ → `数値AI_演習.py` を **貼り付けて** 🔮 推論 |

> **書く場所＝手元の `数値AI_演習.py`**（VSCode等。全体が見える平文）。
> 書けたら、その中身を下の欄に**貼り付けて実行**＝計算は **Cloud（サーバ側）** で動きます。
"""
    )
    ...
    st.caption(
        f"**説明変数(X)＝今月の活動**（{', '.join(ACT_COLS)}）。**答え(y)＝翌月の成約数**。"
        f" 全{len(df)}行（{df['営業担当'].nunique()}担当×{int(df['月'].max())}ヶ月）。"
        f" 学習用 {len(feat)} 行（過去{int(cutoff)}ヶ月で学習→残りで検証）。"
    )

    # --- 🟢 Step2: 説明変数を書いて学習（モデル作成＋fit を1セル）---
    st.divider()
    st.subheader("🟢 Step2：学習コードを書いて貼り付け（手元の `数値AI_演習.py` に書く）")
    st.text_area("学習コードを貼り付け（→「🎓 学習」でCloud実行）", key="learn_code", height=240)
    st.button("🎓 学習", type="primary", on_click=_do_learn)
    _cols_disp = "、".join(f"「{c}」" for c in ACT_COLS)
    st.caption(
        f"**手元の `数値AI_演習.py`（全体が見える平文）に書いて**、その中身をここに貼り付け → 「🎓 学習」で **Cloud 実行**。"
        f" 使える活動の列名：{_cols_disp}。（発展）`max_depth=10` などを足すと過学習しやすい。"
    )

    if st.session_state.get("fit_err"):
        st.error(f"⚠️ エラー: {st.session_state.fit_err}\n\nStep2 の書き方を見直してください。")
    elif st.session_state.get("model") is not None:
        st.success(f"学習できました！（説明変数：{', '.join(st.session_state.feature_cols)}）")
        s1, s2 = st.columns(2)
        s1.metric("訓練スコア（train R²）", f"{st.session_state.train_score:.3f}")
        s2.metric("検証スコア（val R²）", f"{st.session_state.val_score:.3f}")
        if st.session_state.train_score - st.session_state.val_score > 0.3:
    ...
    # --- 🟢 Step3: 推論（predict）---
    st.divider()
    st.subheader("🟢 Step3：推論コードを書いて貼り付け（手元の `数値AI_演習.py` に書く）")
    st.selectbox("予測する営業担当を選ぶ", sorted(df["営業担当"].unique()), key="rep_select")
    _, last3 = latest_row(df, st.session_state.rep_select)
    st.caption(f"{st.session_state.rep_select} の直近の活動（最新月＝来月のデータ）:")
    st.dataframe(last3[["月", *ACT_COLS]], width="stretch", hide_index=True)
    st.text_area("推論コードを貼り付け（→「🔮 推論」でCloud実行）", key="predict_code", height=120)
    st.button("🔮 推論", type="primary", on_click=_do_predict)
    st.caption("**手元の `数値AI_演習.py` に書いて**、その中身をここに貼り付け → 「🔮 推論」で **Cloud 実行**。")
    ...
    if st.session_state.get("pred_err"):
        ...
        st.caption("担当や説明変数・モデルの数字を変えて、予測がどう動くか試してみましょう。")


# 🟢 演習用：コード欄は「貼り付け待ち」。書く場所は手元の 数値AI_演習.py。
LEARN_SNIPPET = '''# 数値AI_演習.py の「学習」部分（説明変数・目的変数・model・fit）を
# ここに貼り付けて「🎓 学習」を押してください（Cloud 上で実行されます）。
'''

PREDICT_SNIPPET = '''# 数値AI_演習.py の「推論」部分（予測 = model.predict(...)）を
# ここに貼り付けて「🔮 推論」を押してください（Cloud 上で実行されます）。
'''
