...
| ファイル | 役割 |
|---|---|
| `app_ensyu.py` | **演習用**（参加者に配布・デプロイ）。学習/推論コード欄が空欄（`___`）。1ファイル完結 |
| `app_seikai.py` | **正解用**（講師リファレンス）。記入済みでそのまま動く。裏方・画面は演習用と同一 |
| `学習コード.py` / `推論コード.py` | **手元で書く用の平文テンプレ**。VSCode等で書いて、アプリのコード欄に貼り付け→Cloud実行（演習＝空欄） |
...
**書く場所＝手元の `学習コード.py` / `推論コード.py`**（VSCode等・全体が見える平文）。書いたら配布URLのコード欄に**貼り付けて実行**（計算はCloud）。

1. **Step1 データを見る**（🔒・全件）：手がかり＝今月の活動4種／答え＝翌月の成約数 を確認
2. **Step2 `学習コード.py` を書く**（🟢）：手元の `学習コード.py` に
   ...
   → 中身をアプリの欄に**貼り付け** → 🎓 学習
3. **Step3 `推論コード.py` を書く**（🟢）：担当を選び `pred = model.predict(来月予測用)` を書いて貼り付け → 🔮 推論 → 来月予測
4. **手がかり・数字を変えて観察**：手がかりを「架電だけ」にすると当たらない／提案・面談で当たる。モデルの数字を上げると過学習（train↑/val↓）

> 書く＝手元の `.py`、実行＝Cloud。貼り付けたコードは**サーバ側で実行**（限定名前空間）。`___` のままだとエラー＝書く体験。正解は `app_seikai.py`（コード欄記入済み）。
