...
def _do_predict():
    ...
    予測 = ns.get("予測")
    if 予測 is None:
        st.session_state.pred_err = "予測 に結果が入っていません。"
        st.session_state.prediction = None
        return
...
| 🟢 | Step3 推論コードを書く | 担当を選ぶ → `数値AI_演習.py` を貼り付けて 🔮 推論 |

> **正解版**：コード欄は記入済み。そのまま「🎓 学習」「🔮 推論」を押せば動きます（答え合わせ用）。
"""
    )
    ...
    st.divider()
    st.subheader("🟢 Step2：学習コード（正解・記入済み）")
    st.text_area("学習コード（→「🎓 学習」でCloud実行）", key="learn_code", height=240)
    st.button("🎓 学習", type="primary", on_click=_do_learn)
    _cols_disp = "、".join(f"「{c}」" for c in ACT_COLS)
    st.caption(
        f"正解＝手元 `数値AI_演習.py` の答え。そのまま **Cloud 実行** できます。"
    ...
    st.divider()
    st.subheader("🟢 Step3：推論コード（正解・記入済み）")
    ...
    st.text_area("推論コード（→「🔮 推論」でCloud実行）", key="predict_code", height=120)
    st.button("🔮 推論", type="primary", on_click=_do_predict)
    st.caption("正解＝手元 `数値AI_演習.py` の答え。そのまま **Cloud 実行** できます。")
...
# 🟢 学習（正解・記入済み）= 数値AI_正解.py の学習部分
LEARN_SNIPPET = '''説明変数 = 訓練データ[["問い合わせ数", "架電数", "面談数", "案件提案数"]]
目的変数 = 訓練データ["翌月の成約数"]
model = XGBRegressor(random_state=42)
model.fit(説明変数, 目的変数)
'''

# 🟢 推論（正解・記入済み）= 数値AI_正解.py の推論部分
PREDICT_SNIPPET = '''予測 = model.predict(来月の説明変数)
'''
